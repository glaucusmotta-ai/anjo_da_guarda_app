...
- WhatsApp por Zenvia compatível com o template APROVADO pela Meta/Zenvia,
  com variáveis **{{nome}}** e **{{local_aproximado}}**.
- Novo helper `_local_aproximado_fragment(lat, lon)` que retorna exatamente
  o sufixo exigido pelo template: `?q=LAT,LON` (com ponto decimal e 7 casas).
- Envio WA (contatos do usuário e legado via .env) preenche:
    fields = { "nome": <nome>, "local_aproximado": "?q=<lat>,<lon>" }
  quando houver coordenadas válidas; caso contrário, cai para texto simples
  (se `ZENVIA_WA_SIMPLE=true` ou se não houver `ZENVIA_WA_TEMPLATE_ID`).
- Mantidos os envs usuais e callbackUrl em `ZENVIA_WA_CALLBACK_URL`.

ATENÇÃO: remova qualquer placeholder do arquivo; este código é completo.
"""
...
# NOVO: fragmento usado pelo template aprovado pela Meta/Zenvia
# Gera exatamente "?q=LAT,LON".
...
# ---------------------------------------------------------
# Zenvia (SMS) — remetente com fallback e saneamento
# ---------------------------------------------------------

def _sanitize_sms_alias(alias: str) -> str:
    """Mantém apenas A–Z, a–z, 0–9; remove espaços; limita a 11 chars (regra comum do mercado)."""
    a = (alias or "").strip()
...
def _resolve_sms_sender() -> tuple[str, str]:
    """Retorna (primary, fallback) já saneados. Se o primário ficar inválido, cai no fallback."""
    primary_raw  = os.getenv("ZENVIA_SMS_FROM") or os.getenv("ZENVIA_FROM") or ""
    fallback_raw = os.getenv("ZENVIA_SMS_FROM_FALLBACK") or "glaucusmotta"

    primary  = _sanitize_sms_alias(primary_raw)
    fallback = _sanitize_sms_alias(fallback_raw)

    if not primary or len(primary) < 3:
        primary = fallback
    if not fallback or len(fallback) < 3:
        fallback = "glaucusmotta"  # último recurso
...
def send_sms_zenvia_once(_from: str, to: str, text: str) -> dict:
    ...
    try:
        ...
        logger.info("[SMS] from=%s to=%s status=%s ok=%s resp=%s", _from, to, resp.status_code, ok, raw)
        return {"ok": ok, "status": resp.status_code, "response": raw, "to": to}
    except Exception as e:
...
def send_sms_zenvia_list(text: str) -> list:
    """Envia para todos os números; tenta alias primário e, se falhar, reenvia com fallback."""
    from_primary, from_fallback = _resolve_sms_sender()
    to_list = [x.strip() for x in os.getenv("ZENVIA_SMS_TO_LIST", "").split(",") if x.strip()]

    results = []
    for to_raw in to_list:
        to = _msisdn_clean(to_raw)
        r1 = send_sms_zenvia_once(from_primary, to, text)
        if not r1.get("ok") and from_fallback and from_fallback != from_primary:
            logger.warning("[SMS][retry-fallback] to=%s primary=%s failed; trying fallback=%s",
                           to, from_primary, from_fallback)
            r2 = send_sms_zenvia_once(from_fallback, to, text)
            if r2.get("ok"):
...
def auth_register(payload: RegisterIn):
    ...
    with db() as con:
        ...
        uid = con.execute("SELECT id FROM users ORDER BY id DESC LIMIT 1").fetchone()[0]
    ...
    with db() as con:
        ...
        if telegram:
            for i, label in enumerate(telegram[:3]):
                ...
                cid = con.execute("SELECT last_insert_rowid() AS id").fetchone()[0]
            ...
            with db() as con:
                ...
                if row:
                    con.execute("UPDATE telegram_contacts SET chat_id=?, activated_at=? WHERE contact_id=?",
                                (chat_id, _now(), row[0]))
                    con.execute("UPDATE contacts SET status='active' WHERE id=",)  # fix below
                    # Corrige UPDATE com parâmetro
                    con.execute("UPDATE contacts SET status='active' WHERE id=?", (row[0],))
...
def _contacts_for_user(uid: int) -> Dict[str, List[sqlite3.Row]]:
    ...
    for r in rows:
        out[r[2]].append(r)  # type está na coluna 2
    return out
...
async def api_sos(payload: SosIn):
    ...
    if user_id:
        ...
        # E-mail dos contatos
        if contacts["email"]:
            email_list = [c[3] for c in contacts["email"]]  # value na coluna 3
        ...
        wa_numbers = [c[3] for c in contacts["whatsapp"]]
        if wa_numbers:
            ...
            if use_simple_wa or not template_id:
                ...
                wa_text = f"🚨 ALERTA de {(nome_tpl or '').strip()}\nSituação: {(payload.text or 'sos pessoal').strip()}\nLocalização: {maps_link or ''}".strip()
                tpl_fields = None
            else:
                # modo template aprovado ({{nome}} e {{local_aproximado}})
                tpl_fields = {}
                if nome_tpl:
                    tpl_fields["nome"] = nome_tpl
                if _valid_coords(lat, lon):
                    tpl_fields["local_aproximado"] = _local_aproximado_fragment(float(lat), float(lon))
                wa_text = ""  # não usado quando é template
        ...
        if contacts["telegram"] and CFG.tg_enabled:
            ...
            if any_ok and _valid_coords(lat, lon):
                for i, rr in enumerate(rows):
                    _send_telegram_location_once(rr[0], float(lat), float(lon))
                    if CFG.tg_broadcast_throttle_ms and i < len(rows) - 1:
                        time.sleep(CFG.tg_broadcast_throttle_ms / 1000.0)
...
def live_update(payload: LiveUpdateIn):
    ...
    any_ok = False
    for r in rows:
        res = _edit_telegram_live_once(str(r[0]), int(r[1]), float(payload.lat), float(payload.lon))
        results.append(res)
        any_ok = any_ok or res.get("ok", False)
...
def live_stop(payload: LiveStopIn):
    ...
    any_ok = False
    for r in rows:
        res = _stop_telegram_live_once(str(r[0]), int(r[1]))
        results.append(res)
        any_ok = any_ok or res.get("ok", False)
...
def debug_sms_dlr_recent(limit: int = 20):
    with db() as con:
        rows = con.execute("""
            ...
            LIMIT ?
        """, (limit,)).fetchall()
    return {"rows": [dict(zip([c[0] for c in con.execute('PRAGMA table_info(sms_dlr)')], r)) for r in rows]}
...
def debug_wa_dlr_recent(limit: int = 20):
    with db() as con:
        rows = con.execute("""
            ...
            LIMIT ?
        """, (limit,)).fetchall()
    return {"rows": [dict(zip([c[0] for c in con.execute('PRAGMA table_info(wa_dlr)')], r)) for r in rows]}
